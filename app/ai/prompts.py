...
def get_system_prompt(current_date: date | None = None, hotel_room_types: list[dict] | None = None, hotel_name: str = "", guest_name: str | None = None, guest_nationality: str | None = None, guest_id_number: str | None = None) -> str:
    ...
    if guest_id_number:
        guest_context_parts.append(f"مسجل لدينا أن رقم هويته: *{guest_id_number}*. لا تسأله مجدداً عن رقم هويته في الحجز.")
...
- `add_expense` — تسجيل مصروف (للمالك فقط)
...
- إذا العميل أعطاك رقم يبدأ بـ +966 أو 05 أو +20 أو 01 أو أي كود دولة → هذا رقم **جوال** مش هوية
- رقم الهوية السعودية يبدأ بـ 1 أو 2 ويتكون من 10 أرقام (مثل: 1098765432)
- رقم الإقامة يبدأ بـ 2 ويتكون من 10 أرقام
- رقم الجواز يختلف حسب الدولة ويكون عادة 8-9 أرقام أو حروف
- إذا العميل قال "رقمي" بدون توضيح → اسأله: "هل هذا رقم جوالك ولا رقم هويتك؟"
- لا تخلط أبداً بين رقم الجوال ورقم الهوية
...
### add_expense
{{"intent": "add_expense", "data": {{"amount": 0, "category": "", "description": ""}}}}
